...
public class FileManager : MonoBehaviour
{
    ...
    void Procressing()
    {
        ...
        else if (selectPath == true)
        {
            ...
            {
                string newPath = Path.GetDirectoryName(folderDialog.FileName);
                print("Path: " + newPath);
                path = newPath;
                CaptureScreen();
                currentImageFiles = ImageFilesSetFromPath(path);
    ...
    string GetCapturePath()
    {
        if (version == true)
        {
            return string.Format("{0}/{1}_{2}.{3}", path, prefixName, imageCount, fileType);
        }
        else
        {
    ...
    // Get image file name list from path
    ...
    // Check new tem that dropped in selected path
